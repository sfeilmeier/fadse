...
public class OMOPSO extends Algorithm {
    ...
    private void computeNewPositions() throws JMException {
        for (int i = 0; i < particlesSize_; i++) {
            ...
            for (int var = 0; var < particle.length; var++) {
                particle[var].setValue(Math.round(particle[var].getValue() + speed_[i][var]));
    ...
    public SolutionSet execute() throws JMException, ClassNotFoundException {
        ...
        if (file != null && !file.equals("")) {
            Logger.getLogger(OMOPSO.class.getName()).log(Level.WARNING, "Using a checkpoint file: " + file);
            int i = 0;
            try {
                //read population
                ...
                //read speed - copied from SMPSO
//                if (speedFile != null && speedFile!="") {
//                    speed_ = readCurrentSpeed(speedFile);
//                }

            } catch (IOException ex) {
                Logger.getLogger(OMOPSO.class.getName()).log(Level.SEVERE, "Checkpoint file does not have enough elements to fill the entire population [" + i + "<" + particlesSize_ + "]. Filling it with random individuals");
