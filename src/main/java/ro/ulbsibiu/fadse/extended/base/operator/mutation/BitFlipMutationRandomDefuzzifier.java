 ...
 */
public class BitFlipMutationRandomDefuzzifier extends Mutation {
     ...
     */
    public BitFlipMutationRandomDefuzzifier() {
        try {
     ...
     * Creates a new instance of the Bit Flip mutation operator
     */
    public BitFlipMutationRandomDefuzzifier(Properties properties) {
        this();
    } // BitFlipMutationFuzzy
...
//            System.out.println("RANDOM DEFUZZ");
            ...
            for (int i = 0; i < params.length; i++) {
                ...
                try {
                    ...
                    fillFIS(fis, params);
                    fis.getVariable("out" + params[i].getName()).setDefuzzifier(new DefuzzifierRandom(fis.getVariable("out" + params[i].getName()), 0.8));
                    fis.evaluate();
...
//                            System.out.println(COG);
                            if (counter == 100) {
                                throw new Exception("*******************Impossible to calcualte COG "+COG+"*****************************************");
                            }
                        }
...
//                    Logger.getLogger(BitFlipMutationRandomDefuzzifier.class.getName()).log(Level.INFO, "Something went rong for(" + e.getMessage() + "): " + params[i].getName() + ". Switching to old bit flip mutation");
...
//                            e.printStackTrace();
    ...
    private double computeProbabilityConstant(Variable outputVariable, double probability){
        x = x + 1;
        return probability;
    }
    ...
    private double computeProbabilityGaussian(Variable outputVariable, double probability) {
        ...
        //a = 1-probability
        //b = 0
        //c = 50 //=>when x is approx 500, y is approximative = probabiliy
...
//        System.out.println("Gauss output" + y);
    ...
    public static void main(String[] args) {
        BitFlipMutationRandomDefuzzifier mutation = new BitFlipMutationRandomDefuzzifier();
        for (int i = 0; i < 500; i++) {
